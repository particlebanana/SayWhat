class ProjectCache
  ...
  def self.filter(focus, audience)
    ...
    projects = projects.where(:focus => focus) unless focus == 'Filter by Focus' || focus == ""
    projects = projects.where(:audience => audience) unless audience == 'Filter by Audience' || audience == ""
    projects
  end
